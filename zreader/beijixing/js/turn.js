...
turnMethods = {
	...
	init: function(opt) {
		...
		var p,// 书页索引
			pair,// 单双页
			d = this.data(),
			ch = this.children(),// 书页信息，jquery方法
			l = ch.length;// 一共多少页
		...
		// 设置书页的翻页属性
		for (p = 1; p <= l; p++) {
			pair = p%2==0;// 0 - 右边书页 1 - 左边书页 偶数页在左边
			d.pages[p] = d.pageWrap[p].
				css((pair) ? {'top': 0, 'left': 0} : {'top': 0, 'right': 0}).
					children(":first").// 原始书页信息
						flip({
							next: (pair) ? p-1 : p+1,// 下一页
							page : p,// 当前页
							turn: this,
							duration: opt.duration,// 转换持续时间
							acceleration : opt.acceleration,
							corners: (pair) ? 'backward' : 'forward',// 页脚样式
							back: (pair) ? d.pageObjs[p-1] : d.pageObjs[p+1],// 原始书页
...
flipMethods = {
	...
	init: function(opt) {
		...
		//设置this.data().pageFlip属性=opt
		flipMethods.setData.apply(this, [{'opt': $.extend({}, flipOptions, opt) }]);
		flipMethods._addEvents.apply(this);// 绑定事件
	...
	_cornerActivated: function(e) {
		...
		var corner,
			...
			pos = d.parent.offset(),// 页面位置
			width = this.width(), // 页面宽度
			height = this.height(), // 页面高度
			x = Math.max(0, e[0].pageX-pos.left), // 鼠标x坐标-页面距左边距离
			y = Math.max(0, e[0].pageY-pos.top), // 鼠标y坐标-页面距顶部距离
			csz = d.opt.cornerSize,// 页脚大小
	...
	_addPageWrapper: function() {
		// this 原始书页
		var att,
			d = this.data().pageFlip, // opt
			parent = this.parent();// class为turn-page-wrapper的div
